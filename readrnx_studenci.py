...
def readrnxnav(file):
    ...
    inav = np.zeros((2000, 1))
    n = -1
    with open(file, "r") as f:
        for line in f:
            answer = line.find('END OF HEADER')  # skip header
            if answer != -1:
                break
        for s in f:
            s = s.replace('D', 'E')
...
def readrnxobs(file, time_start, time_end, GNSS):
    with open(file, "r") as f:
        ...
        for s in f:
            label = s[59:]
            if label.find('APPROX POSITION XYZ') == 1:
                RECV = [float(coord) for coord in s.split()[:-3]]
...
nav_file = 'D:\Pliki_Kuby\Studia\II_ROK\SNS\Projekt2\WROC00POL_R_20210600000_01D_GN.rnx'
obs_file = 'D:\Pliki_Kuby\Studia\II_ROK\SNS\Projekt2\WROC00POL_R_20210600000_01D_30S_MO.rnx'
nav, inav = readrnxnav(nav_file)
obs, iobs, recv_coord = readrnxobs(obs_file, CalcTime.start_day, CalcTime.stop_day, 'G')